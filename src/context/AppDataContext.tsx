...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  // Auto-seed schedules if empty on first load
  useEffect(() => {
    if (!loading && schedules.length === 0) {
      const hasSeededSchedules = localStorage.getItem('schedules_seeded');
      if (!hasSeededSchedules) {
        ...
        seedSchedules(augustSchedules).then(() => {
          localStorage.setItem('schedules_seeded', 'true');
        }).catch((err) => {
          console.error('Failed to auto-seed schedules:', err);
        });
      }
    }
  }, [loading, schedules.length, seedSchedules]);
