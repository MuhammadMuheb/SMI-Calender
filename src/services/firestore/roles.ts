...
export async function updateJobRoleDb(
  ...
  if ('shiftStartTime' in updates) mapped.shiftStart = updates.shiftStartTime;
  if ('shiftEndTime' in updates) mapped.shiftEnd = updates.shiftEndTime;
  await updateDoc(doc(db, 'jobRoles', id), mapped);
}
